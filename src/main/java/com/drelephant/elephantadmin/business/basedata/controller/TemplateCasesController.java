...
public class TemplateCasesController extends BaseController {
     ...
     * 例模板状态更细-单条/批量更新状态.
     *
     * @param tmpCodeList tmpCodeList
     * @param status      status
     * @return
     */
    @ApiOperation("病例模板状态更细-单条/批量更新状态")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "tmpCodeList", value = "post请求体,病例编码数组, 例 [code1,code2...]", required = true),
            @ApiImplicitParam(name = "status", value = "url拼接参数, 状态字段, 例如 ?status=xxx", required = true),
    })
    @PostMapping("/updateBatchTemp")
    public R updateBatchStatus(@RequestBody List<String> tmpCodeList, @RequestParam("status") String status) {
        if (StringUtils.isBlank(status)) {
            return R.error("status 必填,拼接status字段到url即可");
        }
        return templateCasesService.updateBatchStatusByCodeList(tmpCodeList, status);
    }
